...
function ct_nav() { ?>
	<nav class="<?php if(is_home()) { echo 'animated fadeInUp'; } ?>">
...
function ct_register_cssjs() {
	...
	wp_register_script('tweenmax', get_template_directory_uri() . '/js/TweenMax.min.js', 'jquery', '1.0', false);
	...
	wp_register_style('preload', get_template_directory_uri() . '/css/preload.css', '', '', 'screen, projection');
...
function ct_wp_head() {
	...
	<style type="text/css">
		...
		<?php if(!is_home()) { echo '#masthead { position: fixed;} section#main-content { margin: 30px 0 0 0;}'; } ?>
		<?php if (Browser::isIE()) { echo '.widget_ct_portfolio ul.portfolio li { width: 259px;}'; } ?>
	</style>
...
function ct_related_posts() {
	...
	if ($tags) {
	  echo '<h4 class="marB20">Related Posts</h4>';
...
function ct_first_image_linked_portfolio() {
    ...
    <figure>
        ...
        <figcaption>
            <h3 class="marB0"><a href="<?php the_permalink() ?>"><?php the_title(); ?></a></h3>
